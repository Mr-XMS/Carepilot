...
import { X, User, Users, FileText, Calendar, Clock, AlertTriangle } from 'lucide-react';
...
export function ShiftDrawer({ shiftId, onClose }: ShiftDrawerProps) {
  ...
  const handleCancel = async () => {
    if (!shift) return;
    if (!window.confirm('Cancel this shift? The worker will need to be notified separately.')) return;
    try {
      await cancelShift.mutateAsync(shift.id);
      onClose();
    } catch {
